...
def get_beam_search_sequences(
    ...
    if isinstance(kwargs, dict):
        kwargs = GenerationConfig(**kwargs)

    if kwargs.return_full_text:
        kwargs.return_full_text = None
    if kwargs.return_dict_in_generate != True:
    ...
    scores = (
        softmax(output.sequences_scores.view(-1, kwargs.num_return_sequences), dim=-1)
        .cpu()
        .numpy()
    )
    ...
    results = []
    for i in num_inputs:
        start = int(i * kwargs.num_return_sequences)
        end = start + kwargs.num_return_sequences

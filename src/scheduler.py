...
def get_cosine_with_hard_restarts__and_decreasing_schedule_with_warmup(
    optimizer: Optimizer, num_warmup_steps: int, num_training_steps: int, num_cycles: int = 1, last_epoch: int = -1, gamma: float = 0.5
):
    lr_lambda = partial(
...
class CosineScheduleTrainer(SFTTrainer):
    ...
    def create_scheduler(self, num_training_steps: int, optimizer: Optimizer = None):
        if self.lr_scheduler is None:
            self.lr_scheduler = get_cosine_with_hard_restarts__and_decreasing_schedule_with_warmup(
